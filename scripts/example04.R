...
msavi_aggr_sample <- msavi_aggr$stats[
  msavi_aggr$stats$id %in% id_sample &
    !is.na(msavi_aggr$stats$med),]
msavi_aggr_sample$id <- factor(msavi_aggr_sample$id, levels = id_sample)
...
msavi_aggr_sample$med <- msavi_aggr_sample$med / 1E4 # from -1000:1000 to -1:1

# Build plot of time series
Sys.setlocale("LC_TIME", "en_GB.UTF-8")
msavi_plot_ts <- ggplot(msavi_aggr_sample, aes(x = date, y = med)) +
  geom_line() + 
  geom_point(aes(colour = orbit, shape = sensor), size = 2) +
  facet_wrap(~id) +
  coord_cartesian(ylim = c(0, 1)) +
  xlab("Date") + ylab("MSAVI") + 
  theme_light()
